// Self-hosted replacement for the old img.shields.io badges. Those pulled
// six images from a third-party service on every page load - if shields.io
// is slow, rate-limited, or blocked by an ad blocker (common: "badge" and
// "shields" patterns are on several default blocklists), the whole contact
// section degraded into broken-image icons with alt text. These are inline
...
function EmailIcon({ className = "h-4 w-4" }) {
  return (
    <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.8" className={className} aria-hidden="true">
      <rect x="3" y="5" width="18" height="14" rx="1.6" strokeLinejoin="round" />
...
function KaggleIcon({ className = "h-4 w-4" }) {
  return (
    <svg viewBox="0 0 24 24" fill="currentColor" className={className} aria-hidden="true">
...
// variant="compact": icon-only circular buttons (Footer). Sized to the same
// 44x44 touch target already used for the mobile nav toggle, with the full
// label as the accessible name rather than printed text.
//
// variant="full": icon + visible label pill (Contact). "only" filters which
// links render, in LINKS order, so Footer can show a subset of Contact's list.
export function SocialLinks({ variant = "full", only, className = "" }) {
  const items = only ? LINKS.filter((l) => only.includes(l.key)) : LINKS;
      ...
      <div className={`flex flex-wrap items-center gap-2 ${className}`}>
        {items.map(({ key, label, href, Icon }) => (
          <a
            ...
            title={label}
            aria-label={label}
            className="flex h-11 w-11 items-center justify-center rounded-full border border-blush text-ink-soft transition-colors hover:border-accent-deep hover:text-accent-deep"
          >
  ...
  return (
    <div className={`flex flex-wrap gap-2 ${className}`}>
      {items.map(({ key, label, detail, href, Icon }) => (
        <a
          key={key}
          href={href}
          target="_blank"
          rel="noreferrer"
          className="inline-flex items-center gap-2 rounded-full border border-blush bg-surface px-4 py-2.5 text-sm text-ink-soft transition-colors hover:border-accent-deep hover:text-accent-deep"
        >
          <Icon />
          <span>
            {label}
